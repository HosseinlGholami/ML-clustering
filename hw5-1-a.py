...
#___________________________________the main program has start_________


#generate random centroid and new one 
cs = [np.array(df.sample()) for x in range(3)] 
...
while(stop==0):
    ...
    if((np.array_equal(cs[0],csn[0]))&(np.array_equal(cs[1],csn[1]))
        ...
        stop=1
        print('algoritm has stoped in %s step',n)
        break
